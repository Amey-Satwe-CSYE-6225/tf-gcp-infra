resource "google_kms_crypto_key_iam_member" "kms-secret-binding" {
  ...
  member        = "serviceAccount:${google_project_service_identity.gcp_sa_cloud_secret_manager.email}"
}


resource "google_project_service_identity" "gcp_sa_cloud_secret_manager" {
